...
public class BaseSpreadActivity extends BaseActivity implements OnClickListener {
	private static final String TAG = BaseSpreadActivity.class.getSimpleName();
	...
	protected void onResume() {
		...
		cardDeck = taro.reading.getCardDeck();
		maxSelectedCards = taro.reading.getNumOfCards();
		currentSelectedCards = cardDeck.getNumberOfCards();
	...
	private void initReadingPlace() {
		...
		for (int i = 1; i <= maxSelectedCards; i++) {
			...
			if (i == 1) {
				...
				cardHeight = Math
						.round((displayHeight > displayWidth ? displayHeight
								: displayWidth) / numberOfRows);
				cardWidth = Math.round(cardHeight * imageWidth / imageHeight);
				lp = new TableRow.LayoutParams(cardWidth, cardHeight, 0x11);
			}

			imageView.setLayoutParams(lp);
			Card card = cardDeck.getCard(i - 1);
			if (card == null) {
				imageView.setVisibility(View.INVISIBLE);
			} else {
	...
	public void onClick(View view) {
		...
		if (taro.cardHelper.cardDeck.isCardSelected(cardIndex))
			...
			cardDeck.addCard(currentSelectedCards,
